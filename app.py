...
ps_category = ['Type', 'Content Rating', 'Genres', 'Current Ver', 'Android Ver']
playstore[ps_category] = playstore[ps_category].astype('category')

# Mengubah kolom Last Updated menjadi date_time dtypes
...
ps_int = ['Reviews', 'Size', 'Installs']
playstore[ps_int] = playstore[ps_int].astype('int64')
...
def index():
    ...
    df2['Total Income'] = df2['Price'] * df2['Installs']
    paid_category = df2.groupby('Category').agg({'Total Income':'sum'}).sort_values(by='Total Income', ascending=False).head(5)
    my_colors = 'rgbkymc'

    paid_category.plot(kind='bar')
    #plt.title('Top 5 Paid Apps with Highest Income')
    plt.xlabel('Category')
    plt.ylabel('Total ($)')
    plt.savefig('paid_category.png',bbox_inches="tight")
